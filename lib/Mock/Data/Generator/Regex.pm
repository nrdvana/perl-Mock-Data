...
sub generate {
	...
	local $self->{max_codepoint}= $opts{max_codepoint} if defined $opts{max_codepoint};
	local $self->{min_codepoint}= $opts{min_codepoint} if defined $opts{min_codepoint};
	local $self->{max_repetition}= $opts{max_repetition} if defined $opts{max_repetition};
	local $self->{prefix}= Mock::Data::Util::coerce_generator($opts{prefix})
		if defined $opts{prefix};
	local $self->{suffix}= Mock::Data::Util::coerce_generator($opts{suffix})
		if defined $opts{suffix};
	my $out= $self->_str_builder($mockdata);
	...
	if ($self->{suffix} && (!$out->next_req || (grep $_ eq "\n", @{ $out->next_req }) && !$out->require->[1])) {
		$str .= "\n" if $out->next_req;
		$str .= $self->{suffix}->();
	}
	return $str;
		...
		# character class?
			...
			if (ord $1 == ord '[') {
				...
				if ($1 eq "\\Z") {
					...
					push @$expr, $self->_charset_node(Mock::Data::Util::charset($1), $flags);
				}
			}
			elsif (ord $1 == ord '.') {
...
sub _node {
	...
	bless { pattern => $pattern, flags => $flags }, 'Mock::Data::Generator::Regex::ParseNode';
}
$INC{'Mock::Data::Generator::Regex::ParseNode'}= __FILE__;
sub Mock::Data::Generator::Regex::ParseNode::flags { $_[0]{flags} }
sub Mock::Data::Generator::Regex::ParseNode::repetition {
	if (@_ > 1) {
...
sub Mock::Data::Generator::Regex::ParseNode::min_repetition {
	$_[0]{repetition}? $_[0]{repetition}[0] : 1
}
sub Mock::Data::Generator::Regex::ParseNode::max_repetition {
	$_[0]{repetition}? $_[0]{repetition}[1] : 1
}
sub Mock::Data::Generator::Regex::ParseNode::pattern { $_[0]{pattern} }
sub Mock::Data::Generator::Regex::ParseNode::generate {
	my ($self, $out)= @_;
	if (my $rep= $self->repetition) {
		my ($min, $n)= ($rep->[0], $out->generator->_random_rep_count($rep));
		for (1 .. $n) {
...
sub _or_node {
	...
	bless { pattern => $or_list, flags => $flags }, 'Mock::Data::Generator::Regex::OrNode';
}
$INC{'Mock::Data::Generator::Regex::OrNode'}= __FILE__;
@Mock::Data::Generator::Regex::OrNode::ISA= ('Mock::Data::Generator::Regex::ParseNode');
sub Mock::Data::Generator::Regex::OrNode::generate {
	my ($self, $out)= @_;
	my ($min, $n)= (1,1);
	if (my $rep= $self->{repetition}) {
		$min= $rep->[0];
		$n= $out->generator->_random_rep_count($rep);
	}
	rep: for (1 .. $n) {
...
sub _charset_node {
	my $self= shift;
	my $flags= pop;
	bless { pattern => Mock::Data::Util::charset(@_), flags => $flags }, 'Mock::Data::Generator::Regex::CharsetNode'
}
$INC{'Mock::Data::Generator::Regex::CharsetNode'}= __FILE__;
@Mock::Data::Generator::Regex::CharsetNode::ISA= ('Mock::Data::Generator::Regex::ParseNode');
sub Mock::Data::Generator::Regex::CharsetNode::generate {
	my ($self, $out)= @_;
	...
	my $fn= $self->{compiled} ||= do {
		my $rep= $self->repetition;
		$self->pattern->min_len($rep? $rep->[0] : 1);
		$self->pattern->max_len($rep? $rep->[1] : 1);
...
sub _assertion_node {
	my $self= shift;
	bless { @_ }, 'Mock::Data::Generator::Regex::AssertionNode';
}
$INC{'Mock::Data::Generator::Regex::AssertionNode'}= __FILE__;
@Mock::Data::Generator::Regex::AssertionNode::ISA= ('Mock::Data::Generator::Regex::ParseNode');
sub Mock::Data::Generator::Regex::AssertionNode::start { $_[0]{start} }
sub Mock::Data::Generator::Regex::AssertionNode::end { $_[0]{end} }
sub Mock::Data::Generator::Regex::AssertionNode::generate {
	my ($self, $out)= @_;
	if ($self->{start}) {
...
sub _str_builder {
	my ($self, $mockdata)= @_;
	bless { mockdata => $mockdata, generator => $self, str => '' },
		'Mock::Data::Generator::Regex::StrBuilder';
}
$INC{'Mock::Data::Generator::Regex::StrBuilder'}= __FILE__;
sub Mock::Data::Generator::Regex::StrBuilder::mockdata { $_[0]{mockdata} } # Mock::Data instance
sub Mock::Data::Generator::Regex::StrBuilder::generator { $_[0]{generator} }
sub Mock::Data::Generator::Regex::StrBuilder::start { $_[0]{start}= $_[1] if @_ > 1; $_[0]{start} }
sub Mock::Data::Generator::Regex::StrBuilder::str { $_[0]{str} } # string being built
sub Mock::Data::Generator::Regex::StrBuilder::require {
	push @{ $_[0]{require} }, $_[1] if @_ > 1;
	return $_[0]{require};
}
sub Mock::Data::Generator::Regex::StrBuilder::next_req {
	return $_[0]{require} && $_[0]{require}[0];
}
sub Mock::Data::Generator::Regex::StrBuilder::append {
	my ($self, $content)= @_;
...
sub Mock::Data::Generator::Regex::StrBuilder::mark {
	my $self= shift;
	my $len= $self->{lastmark}= length $self->{str};
	my $req= $self->{require};
	return [ \$self->{str}, $len, $req? [ @$req ] : undef, $self->start ];
}
sub Mock::Data::Generator::Regex::StrBuilder::reset {
	my ($self, $origin)= @_;
